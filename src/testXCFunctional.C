...
int main(int argc, char **argv)
{
  ...
  xcf_list[2] = new LIBXCFunctional(rh, "LIBXC GGA_X_PBE GGA_C_PBE");
  //xcf_list[2] = new LIBXCFunctional(rh, "LIBXC LDA_X LDA_C_PZ_MOD");

  for ( int ixcf = 0; ixcf < 3; ixcf++ )
  {
    Timer tm;
    ...
    else
    {
      ...
      for ( int i = 0; i < n3; i++ )
      {
        dvxc1sum += vxc1[i];
        dvxc2sum += vxc2[i];
        //cout << vxc1[i] << " " << vxc2[i] << endl;
      }

      // Note: the energy variation is 3 * dExc/da * delta(a)
      cout << dvxc1sum << " " << dvxc2sum << endl;
    }
